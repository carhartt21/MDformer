...
class UnalignedDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        # modified_opt = utils.copyconf(self.opt, load_size=self.opt.crop_size if is_finetuning else self.opt.load_size)
        transform = get_transform(self.opt)
        A = transform(A_img)
        B = transform(B_img)
        return {'Source': A, 'Target': B}

        # return {'Source': A, 'Target': B, 'Source_paths': A_path, 'Target_paths': B_path}
