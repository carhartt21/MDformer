...
class TrainDataset(BaseDataset):
    ...
    def rand_scale_crop(self, img, batch_size):
        ...
        for i in range(self.batch_per_gpu):
            ...
            if (self.rand_crop and max(img.size) > max(batch_size) * 2
                ...
                if self.spatial_mask:
                    img, mask = self.rand_scale_crop(img, batch_size, mask)
                else:
                    img = self.rand_scale_crop(img, batch_size)
            # random_flip
...
class TestDataset(BaseDataset):
    def __init__(self, odgt, opt, multi_scale=False, **kwargs):
