...
def build_scheduler(train_cfg, optimizer, n_iter_per_epoch):
    num_steps = int(train_cfg.EPOCHS * n_iter_per_epoch)
    warmup_steps = int(train_cfg.WARMUP_EPOCHS * n_iter_per_epoch)
    decay_steps = int(train_cfg.LR_SCHEDULER.DECAY_EPOCHS * n_iter_per_epoch)
    multi_steps = [i * n_iter_per_epoch for i in train_cfg.LR_SCHEDULER.MULTISTEPS]

    lr_scheduler = None
    if train_cfg.LR_SCHEDULER.NAME == 'cosine':
        lr_scheduler = CosineLRScheduler(
            optimizer,
            t_initial=(num_steps - warmup_steps) if train_cfg.LR_SCHEDULER.WARMUP_PREFIX else num_steps,
            t_mul=1.,
            ...
            cycle_limit=1,
            t_in_epochs=False,
            warmup_prefix=train_cfg.LR_SCHEDULER.WARMUP_PREFIX,
        )
    ...
    elif train_cfg.LR_SCHEDULER.NAME == 'multistep':
