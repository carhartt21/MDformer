...
class RandomCrop:
    ...
    def __call__(self, sample):
        ...
        _i, _j, _h, _w = transforms.RandomCrop.get_params(
            sample.ref_img, output_size=self.size
        )
...
class RandomScale:
    def __init__(self, min_output_size: tuple):
        ...
        assert isinstance(
            min_output_size, tuple
        ), "Minimum output size must be an tuple"
    ...
    def __call__(self, sample):
        ...
        sample.seg_masks = transforms.functional.resize(
            sample.seg_masks, int(w * scale), interpolation=InterpolationMode.NEAREST
        )
        sample.ref_img = transforms.functional.resize(
            sample.ref_img, int(w_ref * ref_scale)
        )
        return sample
...
class Normalize:
    ...
    def __call__(self, sample):
        ...
        sample.ref_img = transforms.functional.normalize(
            sample.ref_img, self.mean, self.std
        )
        return sample
...
class ToTensor:
    def __call__(self, sample):
        ...
        sample.seg_masks = transforms.functional.pil_to_tensor(sample.seg_masks).to(
            torch.int32
        )
    ...
    def __call__(self, sample):
        ...
        for c in fg_classes:
            ...
            if c in _classes:
                ...
                _boxes = mask_to_bboxes(
                    mask.squeeze(), min_size=self.min_pixel, pos_thres=0.0
                )
                for box in _boxes:
