...
class ToRGB(nn.Module):
    def __init__(
        self, in_channel, upsample=True, resolution=None, blur_kernel=[1, 3, 3, 1]
    ):
        super().__init__()
...
class StyleSwinTransformerBlock(nn.Module):
    ...
    def forward(self, x):
        # TODO: check wether to perform normaization before or after the attention/MLP
        H, W = self.input_resolution
        B, L, C = x.shape
        assert (
            L == H * W
...
class StyleBasicLayer(nn.Module):
    ...
    def forward(self, x):  # TODO: adjust for depth>2
        if self.use_checkpoint:
            x = checkpoint.checkpoint(self.blocks[0], x)
            x = checkpoint.checkpoint(self.blocks[1], x)
        else:
            x = self.blocks[0](x)
            x = self.blocks[1](x)
...
class BilinearUpsample(nn.Module):
    ...
    def __init__(self, input_resolution, dim, out_dim=None):
        ...
        self.sin_pos_embed = blocks.SinusoidalPositionalEmbedding(
...
class SwinGenerator(nn.Module):
    """
    Generator model based on the Swin Transformer blocks.
    It takes an input tensor and generates an image of the desired size.

    Args:
        model_cfg (object): Configuration object for the model.
        channel_multiplier (int, optional): Channel multiplier for the model. Defaults to 1.
        enable_full_resolution (int, optional): Enable full resolution for the model. Defaults to 8.
        use_checkpoint (bool, optional): Whether to use checkpointing during forward pass. Defaults to False.
        qkv_bias (bool, optional): Whether to include bias in the QKV projection. Defaults to True.
        qk_scale (float, optional): Scale factor for QK projection. Defaults to None.
        drop_rate (float, optional): Dropout rate. Defaults to 0.
        attn_drop_rate (float, optional): Attention dropout rate. Defaults to 0.

    Attributes:
        style_dim (int): Dimension of the style input.
        size (int): Size of the generated image.
        mlp_ratio (float): Ratio of MLP hidden size to input size.
        layers (nn.ModuleList): List of StyleBasicLayer modules.
        to_rgbs (nn.ModuleList): List of ToRGB modules.
        n_latent (int): Number of latent layers in the model.

    Methods:
        forward(x): Performs forward pass of the model.
        flops(): Calculates the number of floating point operations in the model.
    """
    ...
    def __init__(
        ...
        super().__init__()
        self.style_dim = model_cfg.style_dim
        self.size = model_cfg.img_size[0]  # size of generate image equal to inp
        self.mlp_ratio = model_cfg.SWIN.mlp_ratio
        ...
        for i_layer in range(start, end + 1):
            ...
            layer = StyleBasicLayer(
                ...
                norm=blocks.AdaptiveInstanceNorm2d,
            )
            self.layers.append(layer)

            out_dim = (
                in_channels[i_layer - start + 1]
                if (i_layer < end)
                else in_channels[i_layer - start]
            )
